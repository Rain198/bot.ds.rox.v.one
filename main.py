...
async def db_init():
    async with aiosqlite.connect(DB_FILE) as db:
        ...
        -- Admin grants (product access without issuing a new key)
        CREATE TABLE IF NOT EXISTS grants(
            ...
            expires_at  INTEGER NOT NULL,
            enabled     INTEGER NOT NULL DEFAULT 1,
            UNIQUE(discord_id, product)
        );

        CREATE INDEX IF NOT EXISTS idx_grants_enabled_expires ON grants(enabled, expires_at);
        CREATE INDEX IF NOT EXISTS idx_grants_discord ON grants(discord_id);
        """)
        await db.commit()


async def db_upsert_grant(discord_id: int, product: str, expires_at: int, enabled: int = 1):
    now = now_ts()
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            """
            INSERT INTO grants(discord_id, product, created_at, expires_at, enabled)
            VALUES(?,?,?,?,?)
            ON CONFLICT(discord_id, product)
            DO UPDATE SET expires_at=excluded.expires_at, enabled=excluded.enabled
            """,
            (discord_id, product, now, expires_at, enabled),
        )
        await db.commit()


async def db_disable_grant(discord_id: int, product: str):
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
            "UPDATE grants SET enabled=0 WHERE discord_id=? AND product=?",
            (discord_id, product),
        )
        await db.commit()


async def db_get_products_for_access(discord_id: int, hwid_hash: str, now: int) -> List[str]:
    """Union of:
    - active, non-expired keys bound to this HWID
    - active, non-expired admin grants for this discord_id
    """
    ...
    async with aiosqlite.connect(DB_FILE) as db:
        ...
        cur = await db.execute(
            ...
            FROM keys
            WHERE active=1 AND expires_at>? AND hwid=?
            """,
            (now, hwid_hash),
        )
        for r in await cur.fetchall():
            products.add(str(r["product"]))
        ...
        cur = await db.execute(
            ...
            SELECT DISTINCT product
            FROM grants
            WHERE enabled=1 AND expires_at>? AND discord_id=?
            """,
            (now, discord_id),
        )
        for r in await cur.fetchall():
...
@admin_only()
async def grant_cmd(ctx: commands.Context, product: str, *args):
    """Выдать продукт без нового ключа (привязка по discord_id).
    ...
    # args may contain duration and/or mention
    for a in args:
        if isinstance(a, discord.Member):
            member = a
        elif isinstance(a, str) and KEY_DURATION_RE.match(a):
            duration_text = a

    if member is None and ctx.message.mentions:
        member = ctx.message.mentions[0]
    if member is None:
        await ctx.reply("❌ Укажи пользователя: !grant fish 30d @User", mention_author=False)
        return
    ...
    exp = now_ts() + sec
    await db_upsert_grant(int(member.id), product, exp, enabled=1)
    await ctx.reply(f"✅ Выдан доступ **{product}** для {member.mention} до <t:{exp}:f>", mention_author=False)


@bot.command(name="revoke")
@admin_only()
async def revoke_cmd(ctx: commands.Context, product: str, member: discord.Member):
    """Отозвать выданный через !grant доступ."""
    product = (product or "").lower().strip()
    if product not in VALID_PRODUCTS:
        await ctx.reply(f"❌ Неизвестный продукт. Доступно: {', '.join(sorted(VALID_PRODUCTS))}", mention_author=False)
        return
    await db_disable_grant(int(member.id), product)
    await ctx.reply(f"🗑️ Доступ **{product}** отозван у {member.mention}", mention_author=False)
...
async def api_login(request: web.Request) -> web.Response:
    ...
    # Access is HWID-based for keys + discord_id-based for admin grants
    products = await db_get_products_for_access(int(row["discord_id"]), hwid_hash, n)
...
async def api_refresh(request: web.Request) -> web.Response:
    """GET /api/refresh
    Returns current products for this discord_id + hwid.
    """
    ...
    hwid_hash = str(payload.get("hwid", "")).strip()
    if not discord_id or not hwid_hash:
        return web.json_response({"ok": False, "error": "BAD_TOKEN"}, status=401)

    products = await db_get_products_for_access(discord_id, hwid_hash, n)
    ...
    return web.json_response({"ok": True, "products": products, "server_time": n, "token": token})
