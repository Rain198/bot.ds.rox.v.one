...
CLIENTS_CATEGORY_NAME = (os.getenv("CLIENTS_CATEGORY_NAME") or "clients").strip()
ADMIN_ROLE_NAME = (os.getenv("ADMIN_ROLE_NAME") or "ADMINISTRATOR").strip()

TOKEN_TTL_SECONDS = int(os.getenv("TOKEN_TTL_SECONDS", "1800"))  # 30 min

# Optional upstream header when fetching script from URL storage
...
def tr(lang: str, ru: str, en: str) -> str:
    return ru if (lang or "ru").lower() == "ru" else en
...
async def db_ensure_user(discord_id: int) -> None:
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
...
async def db_get_user(discord_id: int) -> Optional[aiosqlite.Row]:
    async with aiosqlite.connect(DB_FILE) as db:
...
async def db_set_lang(discord_id: int, lang: str) -> None:
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute("UPDATE users SET lang=? WHERE discord_id=?", (lang, discord_id))
        await db.commit()


async def db_set_user_channel(discord_id: int, channel_id: int) -> None:
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute("UPDATE users SET user_channel_id=? WHERE discord_id=?", (channel_id, discord_id))
        await db.commit()


async def db_get_user_by_auth(auth_code: str) -> Optional[aiosqlite.Row]:
    async with aiosqlite.connect(DB_FILE) as db:
        db.row_factory = aiosqlite.Row
...
async def db_set_auth(discord_id: int, auth_code: str) -> None:
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute("UPDATE users SET auth_code=? WHERE discord_id=?", (auth_code, discord_id))
        await db.commit()


async def db_set_hwid(discord_id: int, hwid_hash: str) -> None:
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute("UPDATE users SET hwid=? WHERE discord_id=?", (hwid_hash, discord_id))
        await db.commit()


async def db_add_sub(discord_id: int, product: str, created_at: int, expires_at: int) -> None:
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
...
async def db_add_script(product: str, filename: str, storage_type: str, storage_path: str, sha256: Optional[str]) -> None:
    async with aiosqlite.connect(DB_FILE) as db:
        await db.execute(
...
async def db_list_scripts_for_products(products: List[str]) -> List[aiosqlite.Row]:
    if not products:
        return []
    q = ",".join(["?"] * len(products))
    async with aiosqlite.connect(DB_FILE) as db:
...
async def db_get_script(script_id: int) -> Optional[aiosqlite.Row]:
    async with aiosqlite.connect(DB_FILE) as db:
        db.row_factory = aiosqlite.Row
...
class LangView(discord.ui.View):
    ...
    async def set_ru(self, interaction: discord.Interaction, _button: discord.ui.Button) -> None:
    ...
    async def set_en(self, interaction: discord.Interaction, _button: discord.ui.Button) -> None:
        await db_ensure_user(interaction.user.id)
...
class GenerateAuthView(discord.ui.View):
    ...
    async def gen_auth(self, interaction: discord.Interaction, _button: discord.ui.Button) -> None:
        await db_ensure_user(interaction.user.id)
...
async def on_member_join(member: discord.Member) -> None:
    try:
        ...
        await ch.send("👋 Добро пожаловать!\nВыберите язык / Choose language:", view=LangView())
...
@admin_only()
async def panel_cmd(ctx: commands.Context, member: discord.Member) -> None:
    ch = await ensure_client_channel(member)
...
@admin_only()
async def givesub_cmd(ctx: commands.Context, product: str, duration: str, member: discord.Member) -> None:
...
@admin_only()
async def addscript_cmd(ctx: commands.Context, product: str, filename: str, storage: str, sha256: str = "") -> None:
    product = (product or "").lower().strip()
    if product not in VALID_PRODUCTS:
        await ctx.reply(f"❌ Неизвестный продукт. Доступно: {', '.join(sorted(VALID_PRODUCTS))}", mention_author=False)
        return
...
@admin_only()
async def listscripts_cmd(ctx: commands.Context) -> None:
    async with aiosqlite.connect(DB_FILE) as db:
        db.row_factory = aiosqlite.Row
